...
当前公开仓库包含核心仿真框架、脱敏后的运行脚本示例、轻量级样例数据，以及本地嵌入模型资源。完整数据集和更多系统组件会在整理与匿名化审查后逐步发布。
