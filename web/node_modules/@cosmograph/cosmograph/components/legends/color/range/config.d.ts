import type { RangeLegendColor, RangeColorLegendConfigInterface } from '@cosmograph/ui';
export interface CosmographRangeColorLegendConfigInterface {
    /**
     ...
     * Each step corresponds to a color value derived from the current {@link CosmographConfig.pointColorByFn pointColorByFn} or {@link CosmographConfig.linkColorByFn linkColorByFn}.
     *
     ...
     * Has no effect if {@link useLinksData} is `false` and active point color strategy is {@link PointColorStrategy.PreciseDegree} (won't use quantiles) or {@link PointColorStrategy.Degree} (will always use quantiles).
     */
    ...
    /**
     * The label for the legend. Can be a string or a function
     that returns a string based on an accessor.
     */
    labelResolver?: string | ((accessor?: string) => string);
}
export declare const defaultCosmographRangeColorLegendConfig: CosmographRangeColorLegendConfig;
export interface CosmographRangeColorLegendConfig extends CosmographRangeColorLegendConfigInterface, Omit<RangeColorLegendConfigInterface, 'extent' | 'label'> {
}
