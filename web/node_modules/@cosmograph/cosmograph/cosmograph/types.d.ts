...
import { ColorAccessorFn, CosmographData, SizeAccessorFn, CosmographPointInput, CosmographLinkInput } from "./config/interfaces/data";
 ...
 */
export interface ICosmograph {
    /**
     * Statistical information about current points and links in the graph.
     *
     * @returns An object containing the count of points and links and summaries of points and links data.
     *
     * Summaries include:
     ...
     *
     * @returns The index of the focused point, or `undefined` if no point is focused.
     */
     ...
     *
     * @remarks
     * If {@link CosmographConfig.pointClusterByFn pointClusterByFn} is provided, keys of map will be values of {@link CosmographConfig.pointClusterBy pointClusterBy} after applying {@link CosmographConfig.pointClusterByFn pointClusterByFn}.
     */
    readonly clusterMapping: Map<string, {
        index: number;
        count: number;
    }>;
    /**
     ...
     *
     * @returns An object containing the crossfiltered indices Sets.
     */
     ...
     * @returns The active point color strategy.
     */
    readonly activePointColorStrategy: PointColorStrategyType;
    /**
     * Provides the active point size strategy for the Cosmograph instance.
     * @returns The active point size strategy.
     */
    readonly activePointSizeStrategy: PointSizeStrategyType;
    /**
     * Provides the active link color strategy for the Cosmograph instance.
     * @returns The active link color strategy.
     */
    readonly activeLinkColorStrategy: LinkColorStrategyType;
    /**
     * Provides the active link width strategy for the Cosmograph instance.
     * @returns The active link width strategy.
     */
    readonly activeLinkWidthStrategy: LinkWidthStrategyType;
    /**
     ...
     * @returns The name of the points table.
     */
    readonly pointsTableName: string;
    /**
     * A value that gives information about the running simulation progress.
     * @returns The current progress of the simulation, where 0 represents the start of the simulation and 1 represents the end, or `undefined` if not available.
     */
    readonly progress: number | undefined;
    /**
     * Gets a boolean indicating whether the simulation is currently running.
     * @returns `true` if the simulation is running, `false` if it is not running, or `undefined` if the information is not available.
     */
     ...
     * @remarks Determined on graph init/rebuild. Based on `enableSimulation` config (if set), or defaults to `true` if links exist or points lack layout (pointXBy/pointYBy).
     *
     * The value updates upon graph initialization or rebuild (e.g., after `setConfig` or data loading).
     * @returns `true` if a simulation can run, `false` otherwise.
     */
    readonly isSimulationAvailable: boolean;
    /**
     * Gets the point degrees for the current graph.
     * @returns The point degrees as an array of their degree values in the order they were sent to Cosmograph, or `undefined` if not available.
     */
    readonly pointDegrees: number[] | undefined;
    /**
     * Gets the point directions for the current graph.
     * @remarks
     * The point directions are calculated based on the in-degree and out-degree of the points.
     * The value is 0 if the point has only an outgoing link (source), 1 if it has both incoming and outgoing links (source+target), 2 if it has only an incoming link (target), and 3 if it has no links (not linked).
     * @returns The point directions as an array of their direction values in the order they were sent to Cosmograph, or `undefined` if not available.
     */
     ...
     * @returns The maximum point size, or 0 if not available.
     */
    readonly maxPointSize: number;
    /**
     * Waits for the data upload promise to resolve, if it exists.
     * This method is used to ensure that any necessary data has been uploaded
     ...
     * @returns The callback function for rescaling the x-axis, or `undefined` if not set.
     */
    readonly rescaleXFn: ((x: number) => number) | undefined;
    /**
     * Gets the callback function for rescaling the y-axis.
     * @returns The callback function for rescaling the y-axis, or `undefined` if not set.
     */
    readonly rescaleYFn: ((y: number) => number) | undefined;
    /**
     * Sets the config for the Cosmograph.
     * @param config - Config to be applied to the Cosmograph.
     */
     ...
     * @param polygonalPoints Array of points defining the polygonal polygon.
     * @param addToSelection Optional parameter determining whether to add the points to the current selection.
     */
    selectPointsInPolygon(polygonalPoints: [number, number][], addToSelection?: boolean): void;
    /**
     * Selects points.
     * @param pointIndices Array of point indices to be selected.
     */
    selectPoints(pointIndices: number[] | null): void;
    /**
     * Selects a point and, optionally, its connected points.
     * @param pointIndexBy Selected point index.
     * @param addToSelection Optional parameter determining whether to add the point to the current selection.
     * @param selectConnectedPoints Optional parameter determining whether to also select the connected points.
     */
     ...
     * @param pointIndexBy Selected point index.
     * @param unselectConnectedPoints Optional parameter determining whether to also unselect the connected points.
     */
    unselectPoint(pointIndexBy?: number, unselectConnectedPoints?: boolean): void;
    /**
     * Unselects points by indices.
     * @param indicesToRemove Array of point indices to be unselected.
     */
    ...
    /**
     * Get indices for currently selected links.
     * @returns Array of selected link indices.
     */
    getSelectedLinkIndices(): number[] | undefined;
    /**
     * Starts the simulation.
     * @param alpha Value between 0 and 1. The higher the value, the more energy the simulation gets.
     */
     ...
     * @returns A promise that resolves when the destroy operation is complete.
     */
    destroy(): Promise<void>;
    /**
     * Resets the Cosmograph instance to its initial state.
     * @param total - If true, resets configuration and displays the message div.
     ...
     * @param index Point index to be zoomed in.
     * @param duration Optional duration in milliseconds.
     * @param scale Optional zoom level.
     * @param canZoomOut Optional parameter determining whether the view can zoom out.
     */
     ...
     * @returns Array with X and Y coordinates of the point, or `undefined` if the point index is invalid.
     */
    getPointPositionByIndex(index: number): [number, number] | undefined;
    /**
     * Gets the current X and Y coordinates of the clusters.
     * @returns Array of point cluster positions.
     */
    getClusterPositions(): number[] | undefined;
    /**
     * Gets the current colors for each point.
     * @returns A Float32Array containing RGBA color values for each point, or `undefined` if not available.
     */
    getPointColors(): Float32Array | undefined;
    /**
     * Gets the current sizes for each point.
     * @returns A Float32Array containing size values for each point, or `undefined` if not available.
     */
    getPointSizes(): Float32Array | undefined;
    /**
     * Gets the current colors for each link.
     * @returns A Float32Array containing RGBA color values for each link, or `undefined` if not available.
     */
    getLinkColors(): Float32Array | undefined;
    /**
     * Gets the current widths for each link.
     * @returns A Float32Array containing width values for each link, or `undefined` if not available.
     */
    getLinkWidths(): Float32Array | undefined;
    /**
     * Retrieves sampled point data as arrays with indices and positions.
     * @returns An object containing separate arrays for point indices and their corresponding positions, or `undefined` if not available.
    ...
    /**
     * Gets an array of point positions that are currently tracked by the renderer.
     * Use {@link trackPointPositionsByIndices} to specify which points to track.
     * @returns A flattened array of point positions `[x0, y0, x1, y1, ...]` for tracked points, or `undefined` if not available.
     * @see {@link trackPointPositionsByIndices} to set which points to track
     * @see {@link getTrackedPointPositionsMap} for a more convenient Map-based format
     */
    getTrackedPointPositionsArray(): number[] | undefined;
    /**
     * Gets a Map of point positions that are currently tracked by the renderer.
     * Use {@link trackPointPositionsByIndices} to specify which points to track.
     * @returns A Map where keys are point indices and values are their corresponding X, Y coordinates, or `undefined` if not available.
     * @see {@link trackPointPositionsByIndices} to set which points to track
     * @see {@link getTrackedPointPositionsArray} for a flattened array format
     */
    getTrackedPointPositionsMap(): ReadonlyMap<number, [number, number]> | undefined;
    /**
     * Tracks point positions by indices on each simulation tick.
     * Once tracking is enabled, use {@link getTrackedPointPositionsArray} or {@link getTrackedPointPositionsMap} to retrieve the current positions.
     * @param indices Array of point indices.
     * @see {@link getTrackedPointPositionsArray} to get tracked positions as a flattened array
     * @see {@link getTrackedPointPositionsMap} to get tracked positions as a Map
     */
    trackPointPositionsByIndices(indices: number[]): void;
    /**
     * Centers and zooms the view to fit all points in the scene.
     * @param duration Duration of the animation (default: 250ms).
     * @param padding Padding around the viewport (0 to 1 as 100% of the viewport, default 0.1.
     */
     ...
     * @param coordinates Array of coordinates in the format [x0, y0, x1, y1, ...].
     * @param duration Duration of the animation (default: 250ms).
     * @param padding Padding around the viewport (0 to 1 as 100% of the viewport, default 0.1).
     */
    fitViewByCoordinates(coordinates: number[], duration?: number, padding?: number): void;
    /**
     * Sets focus on a point by index. Draws a ring around the focused point.
     * @param index Index of the point to be focused.
     * @remarks
     ...
     *
     * @param {number[] | null} indices - Array of point indices to pin. Set to `[]` or `null` to unpin all points.
     * @example
     *   // Pin points 0 and 5
     ...
     * @param index The index of the point to get the connected points for.
     * @returns An array of indices of the connected points, or `undefined` if the point index is invalid or there are no connected points.
     */
     ...
     * @param index The index of the point to get the degrees for.
     * @returns An object containing the in-degree (incoming links) and out-degree (outgoing links) for the point,
     ...
     * @param spaceRadius Radius of the point in the space coordinate system.
     * @returns Radius of the point in the screen coordinate system.
     */
    spaceToScreenRadius(spaceRadius: number): number | undefined;
    /**
     * Get point space radius by its index.
     * @param index Index of the point.
     * @returns Radius of the point in the space coordinate system.
     */
     ...
     * @returns A Map where keys are the ids of the points and values are their corresponding X and Y coordinates.
     */
    ...
    /**
     * Gets the id of the active selection source.
     * @returns The id of the active selection source, or `undefined` if no selection is active.
     */
    getActiveSelectionSourceId(): string | undefined;
    /**
     * Gets the points selection object from the crossfilter.
     * Provides access to selection clauses and active selection state.
     */
    pointsSelection: MosaicSelection;
    /**
     * Gets the links selection object from the crossfilter.
     * Provides access to selection clauses and active selection state.
     */
    linksSelection: MosaicSelection;
    /**
     ...
     * @param indices Array of point indices.
     * @returns A Promise resolving to an array of point ids.
     */
     ...
     * @param indices Array of point indices.
     * @returns A Promise resolving to a `CosmographData` table of points.
     */
     ...
     * @param values The values to match in the specified column.
     * @returns A Promise resolving to an array of point indices, or `undefined` if the operation fails.
     */
     ...
     * @param column The column in the points table to filter by.
     * @param value The numeric value to match in the specified column.
     * @param isMax Indicates whether to return the indices for the maximum or minimum value.
     * @returns A Promise resolving to an array of point indices, or `undefined` if the operation fails.
     */
     ...
     * @param column The column in the links table to filter by.
     * @param values The values to match in the specified column.
     * @returns A Promise resolving to an array of point indices.
     */
     ...
     * @param ids Array of point ids.
     * @returns A Promise resolving to a `CosmographData` table of links.
     */
     ...
     * @param data `CosmographData` table.
     ...
     * @param positions An array of tuple positions.
     * @returns A flattened array of coordinates.
     */
     ...
     * @param points The points data to add.
     * @remarks
     ...
     * @param links The links data to add.
     * @remarks
     ...
     * @param removeAttachedLinks Whether to also remove links attached to the points.
     ...
     * @param ids The IDs of the points to remove.
     * @returns A promise that resolves when the points have been removed.
     */
    removePointsByIds(ids: string[]): Promise<void>;
    /**
     * Removes links from the Cosmograph instance by their point ID pairs.
     * @param pairs The point ID pairs of the links to remove.
     * @returns A promise that resolves when the links have been removed.
     */
     ...
     * @param pairs The point indices pairs of the links to remove.
     * @returns A promise that resolves when the links have been removed.
     */
    removeLinksByPointIndicesPairs(pairs: [number, number][]): Promise<void>;
    /**
     * Adds a custom Mosaic component (vgplot, spec, etc.) and mounts it to a container.
     * The component will automatically participate in Cosmograph's crossfilter system.
     * @param component The vgplot component
...
export type CosmographStats = {
    ...
    linksCount: number;
    /**
     * Summary information for the points in the Cosmograph visualization.
     * Stats include column names, column types, number of aggregates over all columns (`count`, `min`, `max`, `approx_unique`, `avg`, `std`, `q25`, `q50`, `q75`),
     * and the percentage of `NULL` values in the columns.
     */
    pointsSummary?: Summary;
    /**
     * Summary information for the links in the Cosmograph visualization (if links are present).
     * Stats include column names, column types, number of aggregates over all columns (`count`, `min`, `max`, `approx_unique`, `avg`, `std`, `q25`, `q50`, `q75`),
     * and the percentage of `NULL` values in the columns.
     */
    linksSummary?: Summary;
